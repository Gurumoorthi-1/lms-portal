    ...
    >
      ...
      <div className="flex items-center justify-between sm:justify-end gap-6">
        ...
        <div className="flex items-center gap-2">
            <button 
              onClick={() => router.push(`/instructor/analytics?examId=${exam._id}`)}
...
export default function InstructorExams() {
  ...
  const filtered = React.useMemo(() => {
    // 1. First, deduplicate by title to ensure instructor only sees one entry per unique exam
    // We prefer templates (no userId) over student instances if both exist
    const sorted = [...exams].sort((a, b) => new Date(b.updatedAt || b.createdAt).getTime() - new Date(a.updatedAt || a.createdAt).getTime());
    
    const uniqueMap = new Map();
    sorted.forEach(e => {
        // Use title + topic as the unique identity
        const key = `${e.title.toLowerCase().trim()}_${e.topic.toLowerCase().trim()}`;
        const existing = uniqueMap.get(key);
        
        // PRIORITY LOGIC for Instructor View:
        // 1. Template (no userId) is ALWAYS prioritized because it's the source.
        // 2. If existing is a student-copy and current is a template, OVERWRITE.
        // 3. Otherwise, if both are same type, prefer the one with "completed" status to show active usage.
        
        const isTemplate = !e.userId;
        const existingIsTemplate = existing && !existing.userId;

        if (!existing) {
            uniqueMap.set(key, e);
        } else if (isTemplate && !existingIsTemplate) {
            // New is template, existing is just a student copy -> Template wins
            uniqueMap.set(key, e);
        } else if (isTemplate === existingIsTemplate) {
            // Both same type, prefer Completed over Pending to show stats
    ...
    // 2. Then apply search and status filters
