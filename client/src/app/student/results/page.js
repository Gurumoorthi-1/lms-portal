...
export default function ResultsPage() {
  ...
  const codingSolved = Object.values(codingResults || {}).filter(s => s.passed).length;
  const totalCoding = Object.keys(codingResults || {}).length || 5;
  const r2Score = Math.round((codingSolved / totalCoding) * 100) || 0;
  
  const interviewAnals = Object.values(interviewResults?.analyses || {});
  const avgIntScore = interviewAnals.length 
    ? interviewAnals.reduce((acc, curr) => acc + (curr.score || 0), 0) / interviewAnals.length
    : 0;
  const r3Score = Math.round(avgIntScore * 10) || 0;
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <div className="flex flex-wrap justify-center gap-6">
            ...
            <div className="bg-white/20 backdrop-blur rounded-2xl px-6 py-4 text-center">
              <div className="text-2xl font-black">{codingSolved}/{totalCoding}</div>
              <div className="text-sm opacity-80">Coding</div>
              <div className="text-lg mt-1">{r2Score >= 60 ? '✅' : '❌'}</div>
            </div>
            <div className="bg-white/20 backdrop-blur rounded-2xl px-6 py-4 text-center">
              <div className="text-2xl font-black">{r3Score}%</div>
              <div className="text-sm opacity-80">Interview</div>
        ...
        <div className="flex flex-wrap gap-4 justify-center pt-4">
          ...
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            onClick={() => { clearSession(); router.push('/student'); }}
